...
function card(record) {
  ...
  return `<article class="review-card" data-id="${esc(record.id)}" data-kind="${esc(record.kind)}">
...
function bindDecisions() {
  document.querySelectorAll('[data-decision]').forEach(button => button.addEventListener('click', async () => {
    const cardEl = button.closest('[data-id]'); const status = button.dataset.decision;
    let note = '';
    if (cardEl.dataset.kind === 'commons-chat-report' && status === 'accepted') {
      const entered = window.prompt('What moderation action was taken? This note is required for an actioned Chat report.', '');
      if (entered === null) return;
      note = entered.trim();
      if (!note) { $('#queueStatus').textContent = 'Add a note describing the moderation action before marking this report actioned.'; return; }
    } else if (status !== 'accepted') {
      note = window.prompt(`${stateLabel(status)} note (optional):`, '') || '';
    }
