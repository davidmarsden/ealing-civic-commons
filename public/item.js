...
function renderCivicMemory(memory) {
  ...
  const primaryEvidence = memoryList(memory.primaryEvidence, 'source', 'No directly linked primary source record has been selected for this story yet.');
  const relatedSources = (memory.relatedSourceMaterial || []).length ? `<div class="memory-subsection"><h4>Related source material</h4><p class="memory-meta">Authoritative records connected through the story’s more specific people, places or organisations.</p>${memoryList(memory.relatedSourceMaterial, 'source')}</div>` : '';
  content.innerHTML = `<div class="civic-memory-heading"><div><p class="eyebrow">Related civic memory</p><h2 id="civicMemoryTitle">What connects to this story?</h2><p>Historical context drawn from Southall Stories reporting and reviewed source material. It adds connections around the story without replacing the original article.</p></div></div><div class="memory-grid"><section class="memory-panel"><h3>People, organisations, places & topics</h3><div class="memory-tags">${entityTags}${topicTags}</div></section><section class="memory-panel"><h3>Earlier reporting</h3>${memoryList(memory.earlierReporting, 'post', 'No earlier article passed the stronger relevance threshold for this story.')}</section><section class="memory-panel"><h3>Evidence & source material</h3><div class="memory-subsection"><h4>Primary evidence</h4><p class="memory-meta">Sources directly cited by this story or used as evidence for a reviewed relationship between entities in it.</p>${primaryEvidence}</div>${relatedSources}</section><section class="memory-panel"><h3>About this context</h3><p class="memory-meta">This historical context is drawn from Southall Stories reporting and reviewed source material. Connections between people, places and topics are generated from the research archive; important relationships and source records are checked before publication.</p></section></div><p class="memory-provenance">Historical context from the Southall Stories research archive. The original publisher remains canonical.</p>`;
