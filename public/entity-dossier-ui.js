...
function matchingPlanningLink(record, route) {
  ...
  const direct = links.find(link => normaliseRoute(link.route) === route);
  if (direct) return direct;
  if (route === 'places/southall-gasworks') {
    return links.find(link => link.rule_id === 'southall-gasworks-2-the-straight') || null;
  }
  return null;
}
