(() => {
  ...
  // Progressive Web App plumbing. Keep this lightweight: Civic Commons remains
  // network-first for civic records, with the service worker providing an app
  // shell/offline fallback rather than a stale mirror of live source data.
  function enablePwa() {
    ...
    if (!document.querySelector('meta[name="theme-color"]')) {
      ...
      theme.content = '#0f4a37';
      document.head.appendChild(theme);
    }
    if ('serviceWorker' in navigator) {
      window.addEventListener('load', () => {
        navigator.serviceWorker.register('/sw.js').catch(error => {
          console.warn('Civic Commons service worker registration failed:', error);
        });
      }, { once: true });
    }
  }
