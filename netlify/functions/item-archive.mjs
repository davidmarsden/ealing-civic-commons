...
export default async request => {
  try {
    const [response, documents, rich, stopTowers, victoriaHall] = await Promise.all([
      feedHandler(request),
    ...
    const combined = [...(data.items || []), ...(documents.items || []), ...(rich.archiveItems || []), ...(stopTowers.archiveItems || []), ...(victoriaHall.archiveItems || [])];
    const seen = new Set();
    ...
    console.log(`Civic item archive complete: ${result.stored} stored; ${result.newCandidates} candidates; ${result.failed} failed; manifest ${result.manifestSize}; rich-source candidates ${rich.archiveItems?.length || 0}; campaign candidates ${campaignCandidates}`);
  } catch (error) {
