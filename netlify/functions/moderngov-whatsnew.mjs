...
function listingUrl(now = new Date()) {
...
function canonicalLink(block, base) {
...
function placeFor(text) {
  const value = String(text).replace(/\bLondon Borough of Ealing(?: Council)?\b/gi, ' ').replace(/\bEaling Council\b/gi, ' ');
  const towns = BOROUGH_TOWNS.filter(town => new RegExp(`\\b${town}\\b`, 'i').test(value));
...
function parseItems(html, base) {
  ...
  while ((match = pattern.exec(html))) {
    ...
    const url = canonicalLink(block, base);
    items.push({
      id: `${SOURCE.id}:${key}`,
      sourceId: SOURCE.id,
      source: SOURCE.name,
...
export async function fetchModernGovWhatsNew() {
  ...
  try {
    ...
    const diagnostics = [{ mode: 'public-html', outcome: 'http-response', httpStatus: response.status, elapsedMs: Date.now() - started }];
    if (!response.ok) {
      const blocked = [401, 403, 406, 429].includes(response.status);
      return { items: [], health: [{ ...SOURCE, ok: false, status: blocked ? 'blocked' : 'error', error: `HTTP ${response.status}`, itemCount: 0, diagnostics }] };
    }
    ...
    return { items: [], health: [{ ...SOURCE, ok: false, status: 'upstream', error: message, itemCount: 0, diagnostics: [{ mode: 'public-html', outcome: 'transport-error', error: message, elapsedMs: Date.now() - started }] }] };
  } finally {
    clearTimeout(timer);
  }
}
