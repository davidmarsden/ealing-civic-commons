...
import { reconcileContributionPublication, listPublishedContributions } from '../lib/public-contributions.mjs';
import { sendDecisionNotification } from '../lib/contributor-notifications.mjs';
...
function configuredToken() { return String(Netlify.env.get('REVIEW_ADMIN_TOKEN') || ''); }
function authorised(request) { const expected = configuredToken(); if (!expected) return false; const header = String(request.headers.get('authorization') || ''); const supplied = header.startsWith('Bearer ') ? header.slice(7) : ''; if (!supplied) return false; const left = Buffer.from(supplied); const right = Buffer.from(expected); return left.length === right.length && timingSafeEqual(left, right); }
...
async function withCanonicalTarget(record, publicationByReview = null) {
  ...
  if (!validItemKey(key)) return { ...record, canonicalTarget:null, publication:null };
  try {
...
async function notifyDecision(record, promotion, request) {
  try { await sendDecisionNotification(record, { promotion, origin:new URL(originFor(request)).origin }); }
  catch (error) { console.error('Contributor decision notification failed', { reviewId:record.id, error }); }
}

export default async request => {
  if (!configuredToken()) return json({ error:'Review queue is not configured. Set REVIEW_ADMIN_TOKEN on this deployment.' }, 503);
  ...
  if (body.action === 'decision') {
    ...
    try {
      const record = await decideReview(body.id, { status:body.status, reviewer:body.reviewer, note:body.note });
    ...
    } catch (error) { return json({ error:error.message || 'Decision failed' }, 400); }
  }

  if (body.action === 'reconcile-publication') {
    try {
      const record = await getReview(body.id); if (!record) return json({ error:'Review item not found' }, 404); if (record.kind !== 'item-contribution') return json({ error:'Only item contributions have a publication state to reconcile' }, 400);
      const result = await reconcileContributionPublication(record.id); const refreshed = await getReview(record.id); if (!refreshed) return json({ error:'Review item not found after reconciliation' }, 404);
      const promotion = promotionPayload(refreshed, result); await notifyDecision(refreshed, promotion, request);
      return json({ ok:true, record:await withCanonicalTarget(refreshed), promotion });
    } catch (error) { console.error('Contribution publication retry failed', { reviewId:body.id, error }); return json({ error:`Public contribution could not be reconciled: ${error.message || error}` }, 500); }
  }

  if (body.action === 'enqueue') { const result = await enqueueReview(body.review || {}); return json({ ok:true, created:result.created, record:await withCanonicalTarget(result.record) }, result.created ? 201 : 200); }
  if (body.action === 'import-public-notices') {
    let candidates; try { candidates = await fetchEalingPublicNoticeCandidates({ limit:body.limit || 16 }); } catch (error) { console.error('Public Notice Portal import failed', error); return json({ error:`Public Notice Portal import failed: ${error.message || error}` }, 502); }
    const results = await Promise.all(candidates.map(candidate => enqueueReview(candidate).catch(error => ({ error }))));
    return json({ ok:true, discovered:candidates.length, created:results.filter(result => result?.created).length, alreadyQueued:results.filter(result => result && !result.error && !result.created).length, failed:results.filter(result => result?.error).length });
  }
